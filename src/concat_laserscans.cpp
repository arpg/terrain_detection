...
Concat_Laserscan::Concat_Laserscan(){
        ...
        ros::NodeHandle nh("~");
        nh.param<std::string>("publish_rate", publish_rate_, 0);
        nh.param<std::string>("num_scan_points", num_scan_points_, 0);
        nh.param<std::string>("right_scan_limit", right_scan_limit_, -0.5);
        nh.param<std::string>("left_scan_limit", left_scan_limit_, 0.5);
...
void Concat_Laserscan::concatScans(const sensor_msgs::LaserScan left_scan, const sensor_msgs::LaserScan right_scan){
    ...
    if((ros::Time::now() - last_left_scan_).toSec() > scan_timeout_s){
    ...
    if((ros::Time::now() - last_right_scan_).toSec() > scan_timeout_s){
